...
class gz::sim::systems::SuctionPluginPrivate
{
    ...
    public: void FreezeLink(const EntityComponentManager &ecm);
...
//////////////////////////////////////////////////
void SuctionPlugin::Reset(const gz::sim::UpdateInfo &/*_info*/,
...
//////////////////////////////////////////////////
void SuctionPluginPrivate::Load(const EntityComponentManager &_ecm,
                                                 const sdf::ElementPtr &_sdf)
{
...
//////////////////////////////////////////////////
void SuctionPluginPrivate::Enable(const bool _value)
{
...
SuctionPlugin::SuctionPlugin()
        : System(), dataPtr(std::make_unique<SuctionPluginPrivate>())
{
}

//////////////////////////////////////////////////
void SuctionPluginPrivate::Update(const UpdateInfo &_info,
                                                                const EntityComponentManager &_ecm)
{
    GZ_PROFILE("SuctionPluginPrivate::Update");
    ...
    // While we are touching, freeze the link
    // this->FreezeLink(_ecm);
...
void SuctionPluginPrivate::FreezeLink(const EntityComponentManager &_ecm)
{
    // gz::sim::Link target = gz::sim::Link(this->targetEntities[0]);
    // target.SetAngularVelocity(_ecm, gz::math::Vector3d::Zero);
    // target.SetLinearVelocity(_ecm, gz::math::Vector3d::Zero);

    gzerr << "Link frozen due to contact.\n";
}

//////////////////////////////////////////////////
void SuctionPluginPrivate::AddTargetEntities(const EntityComponentManager &_ecm,
                                                                                     const std::vector<Entity> &_entities)
{
    if (_entities.empty())
        return;
                                                        ...
                                                        EntityComponentManager &_ecm, EventManager &)
{
    ...
    {
        gzerr << "Suction plugin should be attached to a model entity. "
                     << "Failed to initialize." << std::endl;
        return;
    }
    this->dataPtr->sdfConfig = _sdf->Clone();
    gzerr << "Suction plugin initialized!" << std::endl;
}

//////////////////////////////////////////////////
void SuctionPlugin::PreUpdate(const UpdateInfo &, EntityComponentManager &_ecm)
{
    ...
    // Logic to create joint when collision was detected
    if (this->dataPtr->validConfig && this->dataPtr->touching && !this->isAttached) {
        gzerr << "It's working" << std::endl;
    }
}
